...
class Preprocessing:
    ...
    def preprocess(self, raw_train_texts, train_labels=None, raw_test_texts=None, 
                   test_labels=None, pretrained_WE=True, return_raw=False):
        ...
        # FILTER WORDS 
        vocab_set = set(vocab)
        parsed_train_texts = list()
        for i, text in enumerate(tqdm(train_texts, desc="parsing training texts")):
            tokens = self.tokenizer(text)
            tokens = [t for t in tokens if t in vocab_set]
            parsed_train_texts.append(' '.join(tokens))

        #########
        train_idx = [i for i, text in enumerate(parsed_train_texts) if len(text.split()) >= self.min_term]
        ...
        if raw_test_texts is not None:
            ...
            # FILTER WORDS 
            vocab_set = set(vocab)
            parsed_text_texts = list()
            for i, text in enumerate(tqdm(test_texts, desc="parsing training texts")):
                tokens = self.tokenizer(text)
                tokens = [t for t in tokens if t in vocab_set]
                parsed_text_texts.append(' '.join(tokens))
            #########
            test_idx = [i for i, text in enumerate(parsed_text_texts) if len(text.split()) >= self.min_term]
        ...
        if return_raw:
            ...
            for idx in train_idx:
                train_raws.append(raw_train_texts[idx])
            rst["train_raws"] = train_raws
        ...
        if raw_test_texts is not None:
            ...
            if return_raw:
                test_raws = []
                for idx in test_idx:
                    test_raws.append(raw_test_texts[idx])
                rst['test_raws'] = test_raws
    ...
    def save(self, output_dir, vocab, train_texts, train_bow, word_embeddings, 
             train_labels=None, test_texts=None, test_bow=None, test_labels=None,
             train_raws=None, test_raws=None):
        ...
        if train_raws:
            file_utils.save_text(train_texts, f"{output_dir}/train_raws.txt")
        ...
        if test_texts is not None:
            ...
            if test_raws:
                file_utils.save_text(train_texts, f"{output_dir}/test_raws.txt")
