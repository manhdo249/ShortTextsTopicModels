...
class KNNTM(nn.Module):
    ...
    def __init__(self, vocab_size, num_training_size, training_data, 
        ...
        if os.path.exists(M_coo_dist_path) and os.path.exists(M_cos_dist_path):
            M_cos_dist = np.load(M_cos_dist_path)['arr_0']
            M_coo_dist = np.load(M_coo_dist_path)['arr_0']
        elif eta == 1.0:
            M_cos_dist = np.zeros((num_training_size, num_training_size), dtype=np.float32)
            M_coo_dist = np.zeros((num_training_size, num_training_size), dtype=np.float32)
        else:
            raise("Invalid settings with COO and COS.")
